...
def search_contacts(contacts, search_term):
    ...
    for contact in contacts:
        ...
        phone_lower = contact['phone'].lower()
        if search_term in name_lower or search_term in phone_lower:
...
def delete_contact(contacts, name):
    ...
    for i, contact in enumerate(contacts):
        if contact['name'].lower() == name:
            contacts.pop(i)                         # remove the contact at index i
            return True
    return False
...
def contact_exists(contacts, name):
    ...
    found = find_contact_by_name(contacts, name)
    if found is not None:           # If the result is not None, the contact exists
        return True
    else:
        return False
